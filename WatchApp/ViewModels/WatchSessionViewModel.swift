...
//  TakeCareWatchApp — Session state, logging, pacing for watch main screen.
//
...
final class WatchSessionViewModel {
    ...
    func addDrink(drinkTypeId: String) {
        ...
        let log = DrinkLog(
            ...
            sourceDevice: AppConstants.SourceDevice.watch.rawValue
        )
        drinkLogRepo.add(log: log)
        WatchConnectivityManager.shared.sendLog(id: log.id, sessionId: log.sessionId, timestamp: log.timestamp, drinkTypeId: log.drinkTypeId, standardDrinkUnits: log.standardDrinkUnits, isAlcoholic: log.isAlcoholic, sourceDevice: log.sourceDevice)
        sessionLogs = drinkLogRepo.logs(for: session.id)

        let now = Date()
        let snapshots = sessionLogs.map { DrinkLogSnapshot(timestamp: $0.timestamp, drinkTypeId: $0.drinkTypeId, standardDrinkUnits: $0.standardDrinkUnits, isAlcoholic: $0.isAlcoholic) }
        guard let settings = userSettingsRepo.fetch() else {
            WatchHaptics.playLogSuccess()
            refreshLogsAndPace()
            return
        }
        let settingsSnapshot = UserSettingsSnapshot(
        ...
        for eventToEmit in result.eventsToEmit {
            ...
            if eventToEmit.eventType == AppConstants.PacingEventType.warning.rawValue {
                WatchHaptics.playWarning()
            } else {
                WatchHaptics.playNotification()
            }
        }

        paceStatus = result.paceStatus
        timeSinceLastDrink = RollingWindowHelpers.timeSinceLastAlcoholicDrink(from: now, logs: snapshots)
        if result.eventsToEmit.isEmpty {
            WatchHaptics.playLogSuccess()
        }
    }
    ...
    func startSession() {
        ...
        if let session = currentSession {
            WatchConnectivityManager.shared.sendSession(id: session.id, startedAt: session.startedAt, sensitivityMode: session.sensitivityMode, hydrationCadence: session.hydrationCadence, autoEndTimeoutMinutes: session.autoEndTimeoutMinutes)
        }
        sessionLogs = []
        paceStatus = AppConstants.PaceStatus.good.rawValue
        timeSinceLastDrink = nil
    ...
    func endSession() {
        ...
        WatchConnectivityManager.shared.sendEndSession(sessionId: session.id, endedAt: session.endedAt ?? Date())
        currentSession = nil
        sessionLogs = []
        paceStatus = AppConstants.PaceStatus.good.rawValue
        timeSinceLastDrink = nil
